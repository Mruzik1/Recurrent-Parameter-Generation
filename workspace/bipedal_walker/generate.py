...
def load_model(checkpoint_path: str, dataset):
    """Load trained RPG model."""
...
def generate_policy(model, dataset, leg_length, leg_width, gravity, friction, output_dir="./experiments/bipedal_walker/generated"):
...
def generate_sweep(model, dataset, param_name, start, end, num_steps, output_dir):
    ...
    for val in values:
        ...
        path = generate_policy(model, dataset, output_dir=output_dir, **params)
        paths.append(path)
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="./experiments/bipedal_walker/generated",
    ...
    model = load_model(args.checkpoint, dataset)
    ...
    if args.sweep:
        ...
        if args.start is None or args.end is None:
            ...
            return
        generate_sweep(model, dataset, args.sweep, args.start, args.end, args.num_steps, args.output_dir)

    elif all(v is not None for v in [args.leg_length, args.leg_width, args.gravity, args.friction]):
        # Single generation mode
        generate_policy(model, dataset, args.leg_length, args.leg_width, args.gravity, args.friction, args.output_dir)
